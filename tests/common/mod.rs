...
//! geomrust-type conversions, reused by every `tests/vs_fixtures.rs`-style
//! integration test.
...
/// Converts a `[x, y, z]` array into a [`geomrust::Point3`].
pub fn point3(a: [f64; 3]) -> geomrust::Point3 {
    geomrust::Point3::new(a[0], a[1], a[2])
}

/// Converts a `[x, y, z]` array into a [`geomrust::Vector3`].
pub fn vector3(a: [f64; 3]) -> geomrust::Vector3 {
    geomrust::Vector3::new(a[0], a[1], a[2])
}

/// Converts a [`FrameJson`] into a [`geomrust::Frame3`].
pub fn frame3(f: &FrameJson) -> geomrust::Frame3 {
    geomrust::Frame3::new(point3(f.origin), vector3(f.z_dir), vector3(f.x_dir)).unwrap()
}
...
#[track_caller]
pub fn assert_point3(actual: geomrust::Point3, expected: [f64; 3]) {
...
/// component-wise, within [`assert_close`]'s tolerance.
#[track_caller]
pub fn assert_vector3(actual: geomrust::Vector3, expected: [f64; 3]) {
    assert_close(actual.x, expected[0]);
    assert_close(actual.y, expected[1]);
    assert_close(actual.z, expected[2]);
}
